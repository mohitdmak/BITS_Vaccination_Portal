...
const alternative_check_admin = async (req: RequestType, res: ResponseType, next: any): Promise<void> => {
    try{
        var domain: string = req.headers.referer!;
        if(!domain){
            throw new ERROR.ClientError(ERROR.HttpStatusCode.UNAUTHORIZED_REQUEST, "Not authorized to access this ADMIN api path.", false);
        }
        else{
