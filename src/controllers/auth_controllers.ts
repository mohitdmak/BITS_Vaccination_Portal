        ...
        try{
            ...
            if(!err){
                ...
                try{
                    ...
                    throw new ERROR.BaseError("Oauth2 Failure.", ERROR.HttpStatusCode.NOT_FOUND, "Could not fetch oauth2 userinfo.", false, true);
                }
...
const set_session_data = async (user: any, req: RequestType, res: ResponseType): Promise<void> => {
    try{
        // find for user in db
        var student: STUDENT | null = await Student.findOne({email: user.data.email});
        student == null ? student = await new Student({"name": user.data.name, "email": user.data.email, "pic": user.data.picture}).save() : student;
        var allow_access: string[] = get_allow_access();
